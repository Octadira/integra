...
[![macOS 14+](https://img.shields.io/badge/macOS-14.0%2B-blue?logo=apple&style=flat-square)](https://www.apple.com/macos/)
...
- **🔄 Autonomous Network Recovery & Auto-Healing**: Uses Apple `Network.framework` (`NWPathMonitor`) to detect sleep/wake events and Wi-Fi transitions, automatically reconnecting broken mounts with exponential backoff.
- **🔒 Enterprise-Grade Security**: Passwords and private key passphrases are stored in **Apple Keychain (`Security.framework`)**. Sockets are strictly isolated in `~/.ssh/integra/sockets/` with `0700` permissions.
...
- **⚡ IDE & Terminal Launchers**: Open remote workspaces directly in **VS Code**, **Cursor**, or **Antigravity 2.0 IDE**, and launch sessions in **Terminal.app**, **Ghostty**, **iTerm2**, or **Warp**.

---

## 📥 Installation

1. Download the latest **`Integra-vX.Y.Z.dmg`** from the [GitHub Releases](https://github.com/Octadira/integra/releases).
2. Open the DMG and drag **Integra** into your `Applications` folder.
3. Launch **Integra**.
4. Open the built-in **Dependency Doctor** from the sidebar to install FUSE-T and SSHFS with one click.
